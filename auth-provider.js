...
Auth.registerAuth = function(app) {
  ...
  app.post('/login', function(req, res) {
    ...
    if (authCode) {
      ...
      return res.redirect(
        util.format(
          '%s?code=%s&state=%s',
          decodeURIComponent(req.body.redirect_uri),
          authCode,
          req.body.state
        )
      );
  ...
  app.all('/token', function(req, res) {
    ...
    if ('authorization_code' == grant_type) return handleAuthCode(req, res);
    else if ('refresh_token' == grant_type) return handleRefreshToken(req, res);
    else {
